...
int main(int argc, char** argv)
{
	...
	double abc[3] = {0,0,0}; double step = 0;
	...
	do 
	{
		...
		dx += step*abc[0]; dy += step*abc[1]; dz += step*abc[2];
...
int caltheta(double dx, double dy, double dz,double* Dx, double* Dy, double* Dz, double* C, int n, double abc[3] )
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		CC += calF1(dx,dy,dz,Dx[i],Dy[i],Dz[i],C[i]) * ( dz + Dz[i] ) / lnew;
	}
	if (abs(norm(AA,BB,CC)) == 0)
	{
        abc[0] = 10086;abc[1] = 10086;abc[2] = 10086;
        return -1;
    }
    abc[0] = -AA/norm(AA,BB,CC);
    abc[1] = -BB/norm(AA,BB,CC);
    abc[2] = -CC/norm(AA,BB,CC);
...
double calstep(double dx, double dy, double dz, double* Dx, double* Dy, double* Dz, double* C, int n, double abc[3], double maxstep, int n_step)
{
	...
	{
		F = calF(dx+maxstep*abc[0]*i/n_step, dy+maxstep*abc[1]*i/n_step, dz+maxstep*abc[2]*i/n_step, Dx, Dy, Dz, C, n);
		//cout << i << ":" << F << endl;
		if (F<F_min)
		{
			F_min = F;
