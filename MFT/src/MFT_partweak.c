...
void warnfunc()
{
	cout << "Usage: ./MFT_partweak [tpname] [tptime] [tpsac dir] [tp arrival time list] "
		 << "[tpcutb] [tpcute] [time of scan start] [time of scan end] [nsigma] [list of sac file for scan]"
		 << "[weakThres]"
		 <<endl;
	cout << "       ./MFT_partweak [tpname] [tptime] [tpsac dir] [tp arrival time list] "
		 << "[tpcutb] [tpcute] [time of scan start] [time of scan end] [nsigma] [list of sac file for scan] "
		 << "[weakThres] -plot [directory for save sac file for plot]"
		 <<endl;
}
void helpfunc()
{
	...
	cout << "[weakThres]: threshold to extend CC peak";
}
...
int main(int argc, char** argv)
{
	...
	if (argc == 14 && strcmp(argv[12],"-plot") == 0)
	{
		strcpy(sac4plotdir,argv[13]);
		isplot = TRUE;
	}
	else if(argc == 12)
	{
	...
	double peaktop_min = atof(argv[11]);
	...
	for (int i = 0; i < n_comp; i++)
	{
		...
		if (peaktop >= peaktop_min) sacstackweak->sacdata[0] += peaktop;
		else sacstackweak->sacdata[0] += sacCC[i]->sacdata[0+weak_nMAX-1];
		for(int j = 1; j < sacstackweak->sachead->npts; j++)
		{
			...
			if (peaktop >= peaktop_min) sacstackweak->sacdata[j] += peaktop;
			else sacstackweak->sacdata[j] += sacCC[i]->sacdata[j+weak_nMAX-1];
		}
	}
	for (int j = 0; j < sacstackweak->sachead->npts; j++) sacstackweak->sacdata[j] /= n_comp;
	//write_sac_double("tmpstack.sac",sacstack);
	//write_sac_double("tmpstackweak.sac",sacstackweak);
	...
	int n_evtdet = detect_event(sacstackweak, mean, sigma, nsigma, n_comp, scntimestart, scntimefinish, evt_detect_lst, eventname);
	...
	review_event_partweak(evt_detect_lst, n_evtdet, sacCC, sacevt, sacscn, n_comp, weak_nMAX, peaktop_min);
